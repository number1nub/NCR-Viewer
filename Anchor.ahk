/*!
	...
	Remarks:
		...
		However if the control had been created with DllCall() and has its own parent window, the container AutoHotkey created
		GUI must be made default with the +LastFound option prior to the call.
	Example:
		> Anchor(ctlID, "xy")    ; bounds a control to the bottom-left edge of the window
		> Anchor(ctlID, "w0.5")  ; Change in GUI width resizes the control width byon a 2:1 ratio
		> Anchor(ctlID, "h")     ; similar to above but directrly proportional to height
